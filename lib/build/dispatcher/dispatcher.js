...
class Dispatcher {
  ...
  constructor(
    ...
    this.uploadToFS = config.uploadToFS;
    /* generate */
  ...
  async run() {
    ...
    try {
      ...
      if (this.uploadToFS.length > 0) {
        feedback.postbuild.info('uploading to FS...');

        this.uploadToFS.forEach((path) => {
          try {
            copyDirectory(path, `.edge/storage/${path}`);
          } catch (error) {
            feedback.postbuild.error(error.message);
            process.exit(1);
          }
        });
      }
