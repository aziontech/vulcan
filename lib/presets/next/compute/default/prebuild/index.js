...
import { mapAndAdaptFunctions } from './mapping/index';
import { assetsPaths } from './mapping/assets';
import { processVercelOutput } from './mapping/process-mapping';
import { getNextProjectConfig } from '../../utils/next';
...
async function run(prebuildContext) {
  ...
  return {
    ...
    filesToInject: [
      ...
      `${getAbsoluteLibDirPath()}/presets/next/compute/default/handler/async-local-storage.js`,
      // node custom server
      `${getAbsoluteLibDirPath()}/presets/next/compute/node/handler/index.js`,
