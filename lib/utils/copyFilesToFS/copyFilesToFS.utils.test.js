...
import fs from 'fs';
import copyFilesToFS from './copyFilesToFS.utils';
...
describe('copyFilesToFS', () => {
  ...
  it('should copy directories to .edge/storage and ignore the prefix', () => {
    const dirs = ['dir1', 'dir2'];
    copyFilesToFS(dirs, 'dir2');
    ...
    expect(fs.existsSync('.edge/storage/file2.txt')).toBe(true);
    expect(fs.readFileSync('.edge/storage/file2.txt', 'utf8')).toBe('content2');

    expect(fs.existsSync('.edge/storage/subdir/file3.txt')).toBe(true);
    expect(fs.readFileSync('.edge/storage/subdir/file3.txt', 'utf8')).toBe(
      'content3',
    );
  });
});
